...
# Envía un RPC de game_table solo a los jugadores de la sala dada.
func _table_rpc(rs: RoomState, method: StringName, p1 = null, p2 = null, p3 = null, p4 = null) -> void:
...
func reset(rs: RoomState = null) -> void:
	if rs: rs.passives.clear()

func _notify(player_id: int, msg: String) -> void:
	_table_rpc_id(player_id, "client_toast", msg)

func _log(rs: RoomState, msg: String, color: Color = Color.WHITE) -> void:
	_table_rpc(rs, "client_log_event", msg, color)

func _player_name(rs: RoomState, player_id: int) -> String:
	if rs.players.has(player_id):
		return rs.players[player_id].name
	return "?"
...
func resolve_on_play(card_data: CardData, acting_player_id: int, rs: RoomState) -> void:
	for effect in card_data.effects:
		if effect.trigger == GameEnums.Trigger.ON_PLAY:
			await _execute_effect(effect, card_data, acting_player_id, rs)

func resolve_on_enter_stable(card_data: CardData, owner_id: int, rs: RoomState) -> void:
	rs.passives.on_card_entered_stable(owner_id, card_data)
	if card_data.is_unicorn():
		await _on_unicorn_stable_changed(owner_id, true, rs)
	if card_data.is_unicorn() and rs.passives.unicorn_effects_disabled(owner_id):
		return
	for effect in card_data.effects:
		if effect.trigger == GameEnums.Trigger.ON_ENTER_STABLE:
			await _execute_effect(effect, card_data, owner_id, rs)

func resolve_on_turn_start(player_id: int, rs: RoomState) -> void:
	var player: PlayerData = rs.players.get(player_id)
	if not player: return
	var stable_copy: Array[CardData] = []
	for c in player.stable:
		stable_copy.append(c)
	for card in stable_copy:
		if card.is_unicorn() and rs.passives.unicorn_effects_disabled(player_id):
			continue
		for effect in card.effects:
			if effect.trigger == GameEnums.Trigger.ON_TURN_START:
				...
				await _execute_effect(effect, card, player_id, rs)

func resolve_on_destroy(card_data: CardData, owner_id: int, rs: RoomState) -> void:
	if card_data.is_unicorn() and rs.passives.unicorn_effects_disabled(owner_id):
		return
	for effect in card_data.effects:
...
func _execute_effect(effect: CardEffect, source_card: CardData, acting_player_id: int, rs: RoomState) -> void:
	match effect.condition:
		GameEnums.Condition.MOVE_UNICORN_TO_OPPONENT:
			await _custom_unicorn_swap(acting_player_id, rs)
			return
		GameEnums.Condition.RETARGET_UPGRADE_DOWNGRADE:
			await _custom_retarget(acting_player_id, rs)
			return

	if effect.has_cost():
		var paid = await _request_pay_cost(effect, acting_player_id, rs)
		if not paid:
			return
	...
	if effect.condition == GameEnums.Condition.CHOICE_EITHER and effect.has_secondary:
		...
		if choice == 0:
			await _execute_action(effect.primary_action_type, effect.primary_amount,
				...
				source_card, acting_player_id, effect, rs)
		elif choice == 1:
...
func _execute_action(
	...
	source_card: CardData, acting_player_id: int, effect: CardEffect, rs: RoomState
) -> bool:
	match action:
		GameEnums.Action.DRAW:
			await _act_draw(amount, acting_player_id, rs)
			return true
		GameEnums.Action.DISCARD:
			await _act_discard(amount, scope, filter, acting_player_id, rs)
			return true
		GameEnums.Action.DESTROY:
			return await _act_destroy(amount, scope, filter, acting_player_id, source_card, rs)
		GameEnums.Action.SACRIFICE:
			return await _act_sacrifice(amount, scope, filter, acting_player_id, source_card, rs)
		GameEnums.Action.STEAL:
			return await _act_steal(amount, scope, filter, acting_player_id, source_card, rs)
		GameEnums.Action.PULL:
			await _act_pull(amount, scope, acting_player_id, effect, rs)
			return true
		GameEnums.Action.SWAP_HANDS:
			await _act_swap_hands(acting_player_id, rs)
			return true
		...
		GameEnums.Action.SUMMON:
			return await _act_summon(amount, zone, filter, acting_player_id, rs)
		GameEnums.Action.SEARCH_DECK:
			await _act_search_deck(filter, acting_player_id, effect, rs)
			return true
		...
		GameEnums.Action.EXTRA_TURN:
			GameManager.queue_extra_turn(rs, acting_player_id)
			return true
		GameEnums.Action.EXTRA_ACTION:
			GameManager.grant_extra_action(rs, max(1, amount - 1))
			return true
		_:
...
func _act_draw(amount: int, player_id: int, rs: RoomState) -> void:
	var drawn = GameManager.draw_cards(rs, amount)
	var player = rs.players.get(player_id)
	if not player: return
	for cid in drawn:
...
func _act_discard(amount: int, scope: GameEnums.Scope, filter: GameEnums.Filter, acting_player_id: int, rs: RoomState) -> void:
	var targets := _resolve_player_targets(scope, acting_player_id, rs)
	var multi := targets.size() > 1
	if multi:
		_begin_action_board("Todos descartan una carta", targets, rs)
	for target_id in targets:
		var actual_amount = amount if amount > 0 else 1
		for i in actual_amount:
			var picked = await _request_discard_pick(target_id, filter, rs)
			if picked == -1:
				continue
			var p = rs.players.get(target_id)
			if not p: continue
			for j in range(p.hand.size()):
				if p.hand[j].id == picked:
					p.hand.remove_at(j)
					break
			rs.discard_pile.append(picked)
			_table_rpc(rs, &"client_card_left_hand", target_id, picked)
			_table_rpc(rs, &"client_sync_hand_size", target_id, p.hand.size())
		if multi:
			_mark_action_board_done(target_id, rs)
	if multi:
		_end_action_board(rs)

func _begin_action_board(title: String, player_ids: Array, rs: RoomState) -> void:
	var ids: Array = []
	for pid in player_ids:
		ids.append(pid)
	_table_rpc(rs, "client_show_action_board", title, ids)

func _mark_action_board_done(player_id: int, rs: RoomState) -> void:
	_table_rpc(rs, "client_update_action_board", player_id)

func _end_action_board(rs: RoomState) -> void:
	_table_rpc(rs, "client_hide_action_board")

# ==============================================================================
# 💥 DESTRUIR / SACRIFICAR
# ==============================================================================

func _act_destroy(amount: int, scope: GameEnums.Scope, filter: GameEnums.Filter, acting_player_id: int, source_card: CardData, rs: RoomState) -> bool:
	var did_any = false
	for i in max(1, amount):
		var pick = await _request_stable_target(acting_player_id, scope, filter, true, source_card, rs)
		if pick.is_empty(): return did_any
		var target_owner = pick["owner_id"]
		var target_card_id = pick["card_id"]
		_remove_from_stable(target_owner, target_card_id, true, rs)
		var dcard = CardDatabase.get_card_data(target_card_id)
		if dcard:
			_log(rs, "💥 %s destruyó %s (de %s)" % [_player_name(rs, acting_player_id), dcard.name_es, _player_name(rs, target_owner)], Color(1, 0.5, 0.4))
		did_any = true
	return did_any

func _act_sacrifice(amount: int, scope: GameEnums.Scope, filter: GameEnums.Filter, acting_player_id: int, source: CardData, rs: RoomState) -> bool:
	if amount == GameEnums.AMOUNT_ALL:
		var owners = _resolve_player_targets(scope, acting_player_id, rs)
		for owner_id in owners:
			var player = rs.players.get(owner_id)
			if not player: continue
			var to_remove: Array[int] = []
			for card in player.stable:
				if card.matches_filter(filter) and not (rs.rules.babies_immune and card.is_baby_unicorn()):
					to_remove.append(card.id)
			for cid in to_remove:
				_remove_from_stable(owner_id, cid, false, rs)
		return true

	var did_any = false
	for i in max(1, amount):
		if scope == GameEnums.Scope.SELF and filter == GameEnums.Filter.SELF and source:
			_remove_from_stable(acting_player_id, source.id, false, rs)
			did_any = true
			continue
		var owners = _resolve_player_targets(scope, acting_player_id, rs)
		for owner_id in owners:
			var pick = await _request_stable_target(owner_id, GameEnums.Scope.SELF, filter, false, source, rs)
			if pick.is_empty(): continue
			_remove_from_stable(pick["owner_id"], pick["card_id"], false, rs)
			did_any = true
	return did_any
...
func _act_steal(amount: int, scope: GameEnums.Scope, filter: GameEnums.Filter, acting_player_id: int, source_card: CardData, rs: RoomState) -> bool:
	var did_any = false
	for i in max(1, amount):
		...
		var card_data = await _extract_from_stable(source_owner, card_id, rs)
		if not card_data: continue
		var dest = rs.players.get(acting_player_id)
		if dest:
			dest.stable.append(card_data)
			_table_rpc(rs, &"client_card_entered_stable_visual", acting_player_id, card_id)
			rs.passives.on_card_entered_stable(acting_player_id, card_data)
			if card_data.is_unicorn():
				await _on_unicorn_stable_changed(acting_player_id, true, rs)
			_log(rs, "🤝 %s robó %s a %s" % [_player_name(rs, acting_player_id), card_data.name_es, _player_name(rs, source_owner)], Color(0.9, 0.8, 1.0))
			did_any = true
	GameManager.check_win_condition(rs)
	return did_any

func _act_pull(amount: int, scope: GameEnums.Scope, acting_player_id: int, effect: CardEffect, rs: RoomState) -> void:
	...
	for i in max(1, amount):
		...
		for j in range(p.hand.size()):
			if p.hand[j].id == taken_id:
				...
				break
		if not taken: return
		var acting = rs.players.get(acting_player_id)
		if acting:
...
func _act_swap_hands(acting_player_id: int, rs: RoomState) -> void:
	var opponents = rs.opponents_of(acting_player_id)
	if opponents.is_empty(): return
	var target_id = opponents[0]
	if opponents.size() > 1:
		target_id = await _request_player_pick(acting_player_id, opponents)
		if target_id == -1: return
	var a = rs.players.get(acting_player_id)
	var b = rs.players.get(target_id)
...
func _act_return_to_hand(amount: int, scope: GameEnums.Scope, filter: GameEnums.Filter, acting_player_id: int, source_card: CardData, rs: RoomState) -> bool:
	var did_any = false
	for i in max(1, amount):
		var pick = await _request_stable_target(acting_player_id, scope, filter, false, source_card, rs)
		if pick.is_empty(): return did_any
		...
		if not card_data: continue
		if card_data.is_baby_unicorn() and rs.rules.nursery_is_safe_zone:
			rs.nursery_deck.append(card_id)
			did_any = true
			continue
		var p = rs.players.get(owner_id)
		if p:
...
func _act_revive(amount: int, zone: GameEnums.Zone, filter: GameEnums.Filter, acting_player_id: int, rs: RoomState) -> bool:
	var did_any = false
	for i in max(1, amount):
		var candidates: Array = []
		for cid in rs.discard_pile:
		...
		did_any = true
		rs.discard_pile.erase(picked_id)
		var card_data = CardDatabase.get_card_data(picked_id)
		var p = rs.players.get(acting_player_id)
		if not p: return did_any
		if zone == GameEnums.Zone.STABLE:
			p.stable.append(card_data)
			_table_rpc(rs, &"client_card_entered_stable_visual", acting_player_id, picked_id)
			await resolve_on_enter_stable(card_data, acting_player_id, rs)
			GameManager.check_win_condition(rs)
		else:
			p.hand.append(card_data)
...
func _act_summon(amount: int, zone: GameEnums.Zone, filter: GameEnums.Filter, acting_player_id: int, rs: RoomState) -> bool:
	var did_any = false
	var basics_blocked = rs.passives.basic_unicorns_blocked_against(acting_player_id, rs.players.keys())
	for i in max(1, amount):
		var candidates: Array = []
		var p = rs.players.get(acting_player_id)
		if not p: return did_any
		...
		if zone == GameEnums.Zone.HAND:
			...
			rs.nursery_deck.erase(picked_id)
		p.stable.append(card_data)
		_table_rpc(rs, &"client_card_entered_stable_visual", acting_player_id, picked_id)
		rs.passives.on_card_entered_stable(acting_player_id, card_data)
		await resolve_on_enter_stable(card_data, acting_player_id, rs)
...
func _act_search_deck(filter: GameEnums.Filter, acting_player_id: int, effect: CardEffect, rs: RoomState) -> void:
	if effect.condition == GameEnums.Condition.SCRY_3:
		var top3: Array = []
		for i in 3:
			if rs.deck.is_empty(): break
			top3.append(rs.deck.pop_back())
	...
	if picked == -1: return
	rs.deck.erase(picked)
	var card_data = CardDatabase.get_card_data(picked)
	var p2 = rs.players.get(acting_player_id)
	if p2 and card_data:
...
func _act_shuffle_deck(filter: GameEnums.Filter, acting_player_id: int, rs: RoomState) -> void:
...
func _remove_from_stable(owner_id: int, card_id: int, is_destroy: bool, rs: RoomState) -> void:
	var player = rs.players.get(owner_id)
	if not player: return
	...
	if is_destroy:
		...
		if victim and victim.is_unicorn() and rs.passives.has_black_knight(owner_id):
			var knight_id = rs.passives.source_card_of(owner_id, GameEnums.Condition.REPLACE_TARGET_UNICORN)
			if knight_id != -1 and knight_id != card_id:
				...
				if accept:
					await _remove_from_stable(owner_id, knight_id, false, rs)
					return
	...
	rs.passives.on_card_left_stable(owner_id, card_data)
	await resolve_on_destroy(card_data, owner_id, rs)
	if card_data.is_baby_unicorn() and rs.rules.nursery_is_safe_zone:
		rs.nursery_deck.append(card_id)
		_table_rpc(rs, &"client_card_left_stable", owner_id, card_id)
	elif goes_to_hand:
		var op = rs.players.get(owner_id)
		if op:
	...
	_table_rpc(rs, &"client_sync_deck_counters", rs.deck.size(), rs.discard_pile.size(), rs.nursery_deck.size())
	if card_data.is_unicorn():
		await _on_unicorn_stable_changed(owner_id, false, rs)

func _extract_from_stable(owner_id: int, card_id: int, rs: RoomState) -> CardData:
	var player = rs.players.get(owner_id)
...
func _on_unicorn_stable_changed(owner_id: int, entered: bool, rs: RoomState) -> void:
	if rs.passives.has_barbed_wire(owner_id):
		await _force_discard_one(owner_id, rs)
	if entered and rs.passives.has_tiny_stable(owner_id):
		await _enforce_tiny_stable(owner_id, rs)

func _count_unicorns(owner_id: int, rs: RoomState) -> int:
	var player = rs.players.get(owner_id)
...
func _enforce_tiny_stable(owner_id: int, rs: RoomState) -> void:
	while _count_unicorns(owner_id, rs) > 5:
		var pick = await _request_stable_target(owner_id, GameEnums.Scope.SELF, GameEnums.Filter.UNICORN_CARD, false, null, rs)
		if pick.is_empty(): break
		await _remove_from_stable(pick["owner_id"], pick["card_id"], false, rs)

func _force_discard_one(owner_id: int, rs: RoomState) -> void:
	var picked = await _request_discard_pick(owner_id, GameEnums.Filter.ANY, rs)
	if picked == -1: return
	var p = rs.players.get(owner_id)
	if not p: return
	for j in range(p.hand.size()):
		if p.hand[j].id == picked:
			p.hand.remove_at(j); break
	rs.discard_pile.append(picked)
	_table_rpc(rs, &"client_card_left_hand", owner_id, picked)
	for pid in rs.players:
		_table_rpc_id(pid, "client_sync_hand_size", owner_id, p.hand.size())

func _move_stable_card(from_id: int, to_id: int, card_id: int, rs: RoomState) -> void:
	var to_p = rs.players.get(to_id)
	if not to_p: return
	var card_data = await _extract_from_stable(from_id, card_id, rs)
	if not card_data: return
	to_p.stable.append(card_data)
	rs.passives.on_card_entered_stable(to_id, card_data)
	_table_rpc(rs, &"client_card_entered_stable_visual", to_id, card_id)
	if card_data.is_unicorn():
		await _on_unicorn_stable_changed(to_id, true, rs)

# ==============================================================================
# 🎴 ACCIONES CUSTOM (cartas con mecánica especial)
# ==============================================================================

func _custom_unicorn_swap(acting_player_id: int, rs: RoomState) -> void:
	var mine = await _request_stable_target(acting_player_id, GameEnums.Scope.SELF, GameEnums.Filter.UNICORN_CARD, false, null, rs)
	if mine.is_empty(): return
	var opps = rs.opponents_of(acting_player_id)
	if opps.is_empty(): return
	var target_id = opps[0]
	if opps.size() > 1:
		target_id = await _request_player_pick(acting_player_id, opps)
		if target_id == -1: return
	await _move_stable_card(acting_player_id, target_id, mine["card_id"], rs)
	var stolen = await _request_owner_stable_pick(acting_player_id, target_id, GameEnums.Filter.UNICORN_CARD, rs)
	if stolen == -1: return
	await _move_stable_card(target_id, acting_player_id, stolen, rs)
	GameManager.check_win_condition(rs)

func _custom_retarget(acting_player_id: int, rs: RoomState) -> void:
	var candidates: Array = []
	for pid in rs.players:
		var pl = rs.players[pid]
		for c in pl.stable:
	...
	if dests.size() > 1:
		...
		if dest_id == -1: return
	await _move_stable_card(chosen["owner_id"], dest_id, chosen["card_id"], rs)

# ==============================================================================
# 👥 REQUESTS DE UI (esperan input del jugador activo)
# ==============================================================================

func _request_pay_cost(effect: CardEffect, player_id: int, rs: RoomState) -> bool:
	var msg = ""
	...
	var p = rs.players.get(player_id)
	...
	for cid in result[1]:
		if effect.cost_action == GameEnums.Action.DISCARD:
			...
			_table_rpc(rs, &"client_card_left_hand", player_id, cid)
		elif effect.cost_action == GameEnums.Action.SACRIFICE:
			await _remove_from_stable(player_id, cid, false, rs)
	for pid in rs.players:
		_table_rpc_id(pid, "client_sync_hand_size", player_id, p.hand.size())
	return true

func _request_discard_pick(player_id: int, filter: GameEnums.Filter, rs: RoomState) -> int:
	var p = rs.players.get(player_id)
...
func _request_stable_target(acting_player_id: int, scope: GameEnums.Scope, filter: GameEnums.Filter, exclude_self_immunity: bool, source_card: CardData, rs: RoomState) -> Dictionary:
	var candidates := TargetResolver.find_candidates(acting_player_id, scope, GameEnums.Zone.STABLE, filter, rs.passives, rs)
	var source_is_magic = source_card != null and source_card.is_magic_spell()
	...
	var babies_immune: bool = rs.rules.babies_immune
	var clean: Array = []
	for cand in candidates:
		...
		if babies_immune and data.is_baby_unicorn():
			continue
		if data.is_unicorn() and wants_unicorn and rs.passives.unicorns_are_pandas(owner):
			continue
		if exclude_self_immunity and data.is_unicorn():
			if rs.passives.unicorn_immune_to_destroy(owner):
				continue
...
func _request_player_pick(acting_player_id: int, candidates: Array) -> int:
	if candidates.is_empty(): return -1
...
func _request_owner_stable_pick(viewer_id: int, owner_id: int, filter: GameEnums.Filter, rs: RoomState) -> int:
	var owner = rs.players.get(owner_id)
	if not owner: return -1
	var candidates: Array = []
	for c in owner.stable:
		if c.matches_filter(filter):
			if c.is_unicorn() and filter == GameEnums.Filter.UNICORN_CARD and rs.passives.unicorns_are_pandas(owner_id):
				continue
...
func _resolve_player_targets(scope: GameEnums.Scope, acting_player_id: int, rs: RoomState) -> Array[int]:
	var out: Array[int] = []
	for p in rs.players:
		match scope:
