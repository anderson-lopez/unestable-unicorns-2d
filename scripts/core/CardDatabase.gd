...
func _parse_card_object(data: Dictionary) -> CardData:
	...
	if "en" in data["visual"]["description"]:
		c.description_en = data["visual"]["description"]["en"]
...
func _parse_effect_object(eff_data: Dictionary) -> CardEffect:
	...
	e.order = int(eff_data.get("order", 1))
	e.trigger = DataParser.parse_trigger(eff_data.get("trigger", "none"))
	e.condition = DataParser.parse_condition(eff_data.get("condition", "none"))

	# --- COSTO ---
	if "cost" in eff_data and eff_data["cost"] != null:
		var cost_data = eff_data["cost"]
		e.cost_required = cost_data.get("required", false)
		e.cost_action = DataParser.parse_action(cost_data.get("action", "none"))
		e.cost_amount = int(cost_data.get("amount", 0))
		e.cost_target_type = DataParser.parse_filter(cost_data.get("target_type", "none"))

	# --- ACCIÓN PRINCIPAL ---
	if "primary_action" in eff_data and eff_data["primary_action"] != null:
		var p_act = eff_data["primary_action"]
		e.primary_action_type = DataParser.parse_action(p_act.get("type", "none"))
		e.primary_amount = int(p_act.get("amount", 0))
	...
	if "secondary_action" in eff_data and eff_data["secondary_action"] != null:
		...
		e.secondary_action_type = DataParser.parse_action(s_act.get("type", "none"))
...
# --- Helpers de búsqueda (útiles para EffectProcessor / Search effects) ---

func get_cards_by_type(type: GameEnums.CardType) -> Array[CardData]:
	var result: Array[CardData] = []
	for id in database:
		if database[id].type == type:
